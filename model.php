...
if(mysqli_connect_errno()) die('Keine Verbindung zur Datenbank. ' . mysqli_connect_error());
...
function getUser(){
	...
	if (!isset($_COOKIE['user'], $_COOKIE['password'])) return false;

	$sql = 'SELECT id FROM users WHERE id = '.$_COOKIE['user'].' AND password = "'.$_COOKIE['password'].'"';
	$stmt = $db->prepare($sql); if(!$stmt) return $db->error;
...
function isUser($email){
	if(get_single('id', 'users', 'email', $email)) return true;
	else return false;
}
...
function addUser($email, $password){
	...
	$stmt = $db->prepare($sql); if(!$stmt) die($db->error);
	if(!$stmt->execute()) die($stmt->error);
...
function checkPassword($email, $password){
	...
	if($hash == md5(md5($user).$password)) return $user;
	else return false;
}
...
function get_array($select, $table, $where, $value){
	...
	$stmt->execute();
	$stmt->bind_result($result);
	while($stmt->fetch()){ $results[] = $result; }
	$stmt->close();
...
function delete_and($table, $where1, $value1, $where2, $value2){
	...
	if(is_string($value1)) $value1 = '"'.$value1.'"';
	if(is_string($value1)) $value2 = '"'.$value2.'"';
