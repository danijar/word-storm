$(document).ready(function() {
	...
	// name input
	$('#name').blur(function(){ rename_list(); });
	$('#name').keyup(function(e){ if(e.keyCode==13) clear_input(); });
	...
	function get_lists(){
		...
		$.getJSON("request.php", { call: "getLists" }, function(lists){
			...
			$.each(lists, function(){
				var list = this;
				$.getJSON("request.php", { call: "getListName", list: list }, function(data){
	...
	function load_list(list){
		active_list = list;
		// name
		$.getJSON("request.php", { call: "getListName", list: active_list }, function(name){
			$('#name').val(name);
		});
		// words
		$.getJSON("request.php", { call: "getWords", list: active_list }, function(words){
			// clear
			$('#list ul li').remove();
	...
	function rename_list(){
		...
		// server
		$.getJSON("request.php", { call: "changeListName", list: active_list, name: name }, function(data){
	...
	function save_word(word){
		word = $.trim(word);
		$.get("request.php", { call: "addWord", list: active_list, name: word });
		clear_input();
	}
	...
	$('*').live('contextmenu', function() {
		if($(this).is('#list ul li')){
			var name = $(this).html();
			// client
			$(this).css({width:$(this).css('width'),background:'red'}).html('&nbsp;').stop().animate({width:0,opacity:0},500,function(){$(this).remove();}).css('overflow','visible');
			// server
			$.getJSON("request.php", { call: "getWordId", name: name }, function(word){
				$.get("request.php", { call: "deleteWord", list: active_list, word: word });
			});
		}
		clear_input();
		return false;
	...
	function display_word(word){
		...
		exist = $('#list ul li').filter(function(){ return $(this).html() == word; });
		if(exist.length>0){
			exist.css('background-color','#000');
			setTimeout(function(){ exist.css('background-color','#ccc'); },800);
			return;
		}

		var container = $('#list span:contains('+word.charAt(0).toString().toUpperCase()+')');
		if(container.length<=0) container = $('#list span:contains(#)');
...
function clear_input(){ $('#type').val(""); $('#type').focus(); }
function style_adaption(){style_scaling(); correct_styles(); }
function correct_styles(){ $('#list ul li').css('line-height', $('#list ul li').innerHeight() + 'px'); }
